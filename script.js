...
// Default data with models having individual repair mappings
const DEFAULT_DATA = {
    iPhone: {
        models: [
...
function loadRepairs() {
    ...
    if (currentCategory === '📱 Phones') {
        ...
        if (model) {
            repairs = [
                { name: 'Battery Replacement', recommended: model.battery, comingSoon: false },
                { name: 'Screen Replacement', recommended: model.screen, comingSoon: false },
                { name: 'Back Glass Repair', recommended: model.backGlass || 45, comingSoon: false },
                { name: 'Camera Repair', recommended: model.camera || 40, comingSoon: false },
                { name: 'Water Damage Treatment', recommended: model.waterDamage || 30, comingSoon: false },
                { name: 'Deep Clean', recommended: model.deepClean || 12, comingSoon: false }
            ];
        }
